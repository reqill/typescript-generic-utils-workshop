...
type ObjectEntries<T> = NonNullable<
  {
    [Key in keyof T]: [
      Key,
      Required<T>[Key] extends never ? undefined : Required<T>[Key] // we are creating { key: [key, value] } object and then mapping it to [key, value] tuple using keyof T
    ];
  }[keyof T]
>;
...
type TupleToReadonlyObject<T extends readonly (string | number | symbol)[]> = {
  readonly [key in T[number]]: key; // mapping throu each member of tuple and creating readonly object
};
